window.CmsEditor={
 blocks:[],pageKey:'public_home',title:'',history:[],future:[],
 ...
 snap(){this.history.push(JSON.stringify(this.blocks));if(this.history.length>50)this.history.shift();this.future=[]},
 undo(){if(!this.history.length)return;this.future.push(JSON.stringify(this.blocks));this.blocks=JSON.parse(this.history.pop());this.draw()},
 redo(){if(!this.future.length)return;this.history.push(JSON.stringify(this.blocks));this.blocks=JSON.parse(this.future.pop());this.draw()},
 ...
 add(type){this.snap();this.blocks.push(this.newBlock(type));this.draw()},
 remove(i){this.snap();this.blocks.splice(i,1);this.draw()},
 duplicate(i){this.snap();this.blocks.splice(i+1,0,JSON.parse(JSON.stringify(this.blocks[i])));this.draw()},
 move(i,d){const j=i+d;if(j<0||j>=this.blocks.length)return;this.snap();[this.blocks[i],this.blocks[j]]=[this.blocks[j],this.blocks[i]];this.draw()},
 toggle(i){this.snap();this.blocks[i].visible=!this.blocks[i].visible;this.draw()},
 ...
 async load(key){this.pageKey=key;this.history=[];this.future=[];try{const p=await req(`/cms/pages/${key}`);const v=(p.versions||[])[0];this.title=p.title||'';this.blocks=v?.snapshot?.blocks||[]}catch{this.blocks=[]}this.draw()},
